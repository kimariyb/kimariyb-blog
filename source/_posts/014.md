...
接着打开 KimariDraw，并分别输入 `example` 文件夹下的 `.txt` 文件，比如我想绘制 NMR 谱，我就输入 `example/NMR_curve.txt`。因为要绘制的是单曲线图，所以选择 `1. The Single Spectrum`，接着选择 `1. NMR`，就会弹出图片，如果满意就选 `y`。如果不满意，则可以继续修改 `settings.ini` 直到自己满意为止。下图是绘制的效果，绘制的图片可以在 `example` 文件夹下找到。可以发现使用 KimariDraw 绘制的各类光谱的效果还是不错的，比较美观。

<img src="2.png">

如果想要绘制多曲线图，可以在输入文件路径后，选择 `2. The Multiple Spectrum`，接着选择 `4. UV-Vis`、`y` 就可以保存图片了。

<img src="3.png" width="50%">

在绘制多曲线图时，除了需要配置 `settings.ini` 之外，还需要额外配置 `multi_settings.yaml`，`multi_settings.yaml` 可以配置多曲线图每个曲线的颜色，以及标签。
