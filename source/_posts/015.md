...
接着会在当前目录下输出一个 `spectrum_curve.txt` 以及 `spectrum_spike.txt`。其中前者是我们需要的，后者可以直接删除。接着分别对每一个振动分析的输出文件重复上述操作，就可以得到 4 个 `spectrum_curve.txt`，不过要注意的是，得到一个 txt 就应该先改一个 txt 的名字，省得重名直接覆盖了。也可以使用笔者在最新的 KimariDraw 中 `script` 文件夹下自带的 `GenData.bat` 来批量产生光谱数据，具体操作请浏览本文的第五节。
...
最后介绍一下如何批量使用 Multiwfn 生成 KimariDraw 绘制光谱所需要的数据。在最新的 KimariDraw 的 `script` 文件夹中有一个 `GenData.bat` 脚本，是通过 Sob 老师 `batchspec.bat` 修改来的。
...
接着把 `GenData.bat` 和 `commands.txt` 放在生成的 `out` 文件目录下，双击运行 `GenData.bat`，此时无论当前目录下有多少个 `out` 文件，都会生成同名的 `txt` 文件。比如当前目录下有 `1.out`、`2.out` 和 `3.out` 运行 `GenData.bat` 后就会在当前目录下生成 `1.txt`、`2.txt` 以及 `3.txt`。这些 `txt` 文件都是绘制光谱图所需要的数据，接着就可以使用 Origin 或者 KimariDraw 绘制光谱了。